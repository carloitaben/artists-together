import type { SessionStorage } from "@vercel/remix"
import type { AuthenticateOptions, StrategyVerifyCallback } from "remix-auth"

import { redirect } from "@vercel/remix"
import { Strategy } from "remix-auth"
import { encrypt, decrypt, generateOtp, generateMagicLink } from "./utils.server"
...
export class OTPStrategy<User> extends Strategy<User, OTPVerifyParams> {
  ...
  constructor(options: OTPStrategyOptions<User>, verify: StrategyVerifyCallback<User, OTPVerifyParams>) {
    super(verify)
  ...
  async authenticate(request: Request, sessionStorage: SessionStorage, options: AuthenticateOptions): Promise<User> {
    if (!this.secret) {
      throw new Error("Missing required `secret` option from OTPStrategy constructor.")
    }
  ...
  private async validateOtpEncrypted(sessionOtpEncrypted: string) {
    ...
    if (!dbPayload || typeof dbPayload.code !== "string" || typeof dbPayload.active !== "boolean") {
    ...
    const expiresAt = new Date(
      createdAt.getTime() + (this.codeGeneration.expiresAt ?? this.codeGenerationDefaults.expiresAt)
    )
    const maxAttempts = this.codeGeneration.maxAttempts ?? this.codeGenerationDefaults.maxAttempts
  ...
  private async validateOtp(code: string, sessionOtpEncrypted: string) {
    const { dbPayload, otp, sessionOtp } = await this.validateOtpEncrypted(sessionOtpEncrypted)

    if (otp.code !== code) {
      // Update attempts count.
      await this.invalidateCode(sessionOtpEncrypted, dbPayload.active, dbPayload.attempts + 1)
