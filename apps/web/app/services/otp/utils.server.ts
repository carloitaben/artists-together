...
import otpGenerator from "./otp-generator.server"
import type { CodeGenerationOptions, MagicLinkGenerationOptions } from "./index.server"
...
export function getBaseUrl(request: Request) {
  ...
  const protocol = host.match(/(:?\.local|^localhost|^127\.\d+\.\d+\.\d+)(:?:\d+)?$/) ? "http" : "https"
